...
- 对 `lottie_embedding` / `lottie_head` 使用独立学习率训练
...
- `train.py`：训练主入口
- `models/lottie_qwen35.py`：Qwen3.5 Lottie 条件生成模型
...
| `--lottie_lr` | `5e-4` | Lottie embedding/head learning rate |
...
当前实现采用 **LoRA + Lottie head** 的训练策略，而不是全参数微调。

优化参数主要分为两组：

1. `lottie_embedding` / `lottie_head`
2. `lora_*`
...
- 使用更高学习率训练与 Lottie token 直接相关的专属模块
- 在有限训练资源下提高适配效率
